            ...
            <div class="row justify-content-center">
                <div class="col-md-10">
                    ...
                    <div class="card shadow-lg border-0">
                        ...
                        <div class="card-body">
                            @can('post-admin')
                            <form action="{{ route('admin.DataAnakStore') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="number" name="nik_ibu" id="nik_ibu" class="form-control @error('nik_ibu') is-invalid @enderror" value="{{ old('nik_ibu') }}" readonly required>
                                        @error('nik_ibu')
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <!-- Pilih Ibu berdasarkan NIK -->
                                        <label for="user_id"><i class="fas fa-user"></i> Pilih Ibu</label>
                                        <select name="user_id" id="user_id" class="form-control @error('user_id') is-invalid @enderror" required>
                                            <option value="">-- Pilih Ibu --</option>
                                            @foreach($users as $user)
                                                <option value="{{ $user->id }}" data-nik="{{ $user->nik }}" {{ old('user_id') == $user->id ? 'selected' : '' }}>
                                                    {{ $user->name }} 
                                                </option>
                                            @endforeach
                                        </select>                                        
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        @elseif(auth()->user()->hasRole('kader'))
                                            {{-- Kader cuma bisa pilih posyandu mereka sendiri, readonly --}}
                                            <input type="text" name="posyandu" id="posyandu" class="form-control" value="{{ auth()->user()->posyandu }}" readonly>
                                        @endif
                                        </div>
                                    </div>
                                </div>
                            ...
                            @can('post-kader')
                            <form action="{{ route('kader.DataAnakStore') }}" method="POST" enctype="multipart/form-data">
                                @csrf
...
<script>
    $(document).ready(function () {
        ...
        // Saat user_id berubah, isi nik_ibu dari data-nik
        $('#user_id').on('change', function () {
            var selectedNik = $(this).find(':selected').data('nik');
            $('#nik_ibu').val(selectedNik || ''); // jika kosong, isi dengan string kosong
        });

        // Jika ada nilai sebelumnya (misalnya setelah validasi gagal), trigger sekali agar nik_ibu langsung terisi
