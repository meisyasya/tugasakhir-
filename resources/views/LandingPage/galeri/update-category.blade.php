        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h1 class="modal-title fs-5" id="modalLabel{{ $item->id }}">Edit Kategori Galeri</h1> <!-- ID label sesuai dengan modal -->
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">

                    <form action="{{ route('admin.CategoryGaleriUpdate', $item->id) }}" method="post"> <!-- Perbaikan route -->
                        @method('PUT')
                        @csrf
                        <div class="mb-3">
